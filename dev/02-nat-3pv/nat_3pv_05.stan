...
data {
  ...
  vector[30] gq_inc_approval; // incumbent approval for generated quantities
}
parameters {
...
generated quantities {
  array[N_inc_status, 2, 30] row_vector[K] prob_gq; // inc_status, third_party, inc_approval, party prob

  for (n in 1:N_inc_status) {
    for (t in 0:1) {
      for (i in 1:30) {
        row_vector[K] beta_approval_agg;
        beta_approval_agg = to_row_vector(beta_inc_approval) + inc_status_approval[n,];

        // apply linear model & convert to probability
        prob_gq[n,t+1,i] = inc_status[n,] + beta_approval_agg*gq_inc_approval[i] + to_row_vector(beta_third_party)*t;
        prob_gq[n,t+1,i] = to_row_vector(softmax(prob_gq[n,t+1,i]'));
      }
    }
  }
}
